...
### Group Devlog
We had an issue with our terrain disappearing, so we tried to look through the history to find the commit where the terrain was deleted so we could revert the commit and try and get it back, however we were unable to do so so instead we had to remake the terrain. To avoid issues with stuff getting deleted, we tried not to work on the project at the same time and informed everyone when we made a commit and to make sure to check that nothing was missing. We also ran into a few merge conflicts where we had to store the changes we made so that we could undo the commit and pull changes before commiting again.
